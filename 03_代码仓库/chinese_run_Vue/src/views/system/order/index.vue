<template>
    <a-card class="activity-container">
        <!-- 查询部分开始 -->
        <div class="data-form">
			<a-form name="menu_controllers" layout="inline" :model="orderController" @finish="onFinish">
				<a-form-item name="orderName" label="订单名称">
					<a-input v-model:value="orderController.name" allow-clear placeholder="输入订单名称查询" />
				</a-form-item>
				<a-form-item>
		...
		<vxe-pager
			background
			v-model:current-page="orderController.currentPage"
			v-model:page-size="orderController.pageSize"
			:total="orderController.totalResult"
...
import { findByPage } from '/@/api/system/order';
export default defineComponent({
    ...
    setup() {
        ...
        const loadData=()=>{
            // 控制表格开始加载数据
            state.loading=true
            findByPage(state.orderController).then(res=>{
                state.orderController.currentPage=res.data.current
                state.orderController.totalResult=res.data.total
                state.orderController.pageSize=res.data.size
        ...
        const handlePageChange:VxePagerEvents.PageChange = ({ currentPage, pageSize })=>{
			state.orderController.currentPage=currentPage
			state.orderController.pageSize=pageSize
			loadData()
		}
        // 控制全选

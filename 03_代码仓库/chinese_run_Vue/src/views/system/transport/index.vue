<template>
    <a-card class="activity-container">
        <!-- 查询部分开始 -->
        <div class="data-form">
			<a-form name="menu_controllers" layout="inline" :model="transportController" @finish="onFinish">
				<a-form-item name="transportName" label="货物名称">
					<a-input v-model:value="transportController.name" allow-clear placeholder="输入货物名称查询" />
				</a-form-item>
				<a-form-item>
		...
		<vxe-table 
			...
			<vxe-column field="driverId" title="司机编号"></vxe-column>
			<vxe-column field="position" title="订单位置"></vxe-column>
		...
		<vxe-pager
			background
			v-model:current-page="transportController.currentPage"
			v-model:page-size="transportController.pageSize"
			:total="transportController.totalResult"
...
import { findByPage } from '/@/api/system/transport';
export default defineComponent({
    ...
    setup() {
        ...
        const loadData=()=>{
            // 控制表格开始加载数据
            state.loading=true
            findByPage(state.transportController).then(res=>{
                state.transportController.currentPage=res.data.current
        ...
        const handlePageChange:VxePagerEvents.PageChange = ({ currentPage, pageSize })=>{
			state.transportController.currentPage=currentPage
			state.transportController.pageSize=pageSize
			loadData()
		}
        // 控制全选

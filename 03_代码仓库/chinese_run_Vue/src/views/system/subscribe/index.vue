<template>
    <a-card class="activity-container">
        <!-- 查询部分开始 -->
        <div class="data-form">
			<a-form name="menu_controllers" layout="inline" :model="subscribeController" @finish="onFinish">
				<a-form-item name="subscribeName" label="客户订单号">
					<a-input v-model:value="subscribeController.name" allow-clear placeholder="输入订单编号查询" />
				</a-form-item>
				<a-form-item>
		...
		<vxe-table 
			...
			<vxe-column field="trancompany" title="华润"></vxe-column>
		...
		<vxe-pager
			background
			v-model:current-page="subscribeController.currentPage"
			v-model:page-size="subscribeController.pageSize"
			:total="subscribeController.totalResult"
...
import { findByPage } from '/@/api/system/subscribe';
export default defineComponent({
    ...
    setup() {
        ...
        const loadData=()=>{
            // 控制表格开始加载数据
            state.loading=true
            findByPage(state.subscribeController).then(res=>{
                state.subscribeController.currentPage=res.data.current
                state.subscribeController.totalResult=res.data.total
                state.subscribeController.pageSize=res.data.size
        ...
        const handlePageChange:VxePagerEvents.PageChange = ({ currentPage, pageSize })=>{
			state.subscribeController.currentPage=currentPage
			state.subscribeController.pageSize=pageSize
			loadData()
		}
        // 控制全选
